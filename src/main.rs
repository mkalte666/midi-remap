...
fn print_midi_event(channel: u8, message: MidiMessage) {
    ...
    match message {
        MidiMessage::NoteOff { key, vel } => {
            eprintln!("NoteOff:{}:{}", key, vel)
        }
        MidiMessage::NoteOn { key, vel } => {
            eprintln!("NoteOn:{}:{}", key, vel)
        }
        MidiMessage::Aftertouch { key, vel } => {
            eprintln!("Aftertouch:{}:{}", key, vel)
        }
        MidiMessage::Controller { controller, value } => {
            eprintln!("Control:{}:{}", controller, value)
        }
        MidiMessage::ProgramChange { program } => {
            eprintln!("ProgramChange:{}", program)
        }
        MidiMessage::ChannelAftertouch { vel } => {
            eprintln!("ChannelAftertouch:{}", vel)
        }
        MidiMessage::PitchBend { bend } => {
            eprintln!("PitchBend:{}", bend.as_f64())
        }
    }
}
    ...
    match event {
        LiveEvent::Midi { channel, message } => {
            ...
            match conn_out.send(&buf[..]) {
                Ok(_) => {}
                Err(e) => {
                    eprintln!("Something went wrong while sending: {}", e)
                }
            }
            if enable_debug_prints {
                eprint!("Put ");
...
fn start_app(args: &clap::ArgMatches, mapper: Mapper) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let port_in = midi_in
        .ports()
        .into_iter()
        .find(|x| midi_in.port_name(x).unwrap() == name_in)
        .ok_or(PortOpenError::from_port(name_in))?;
    let port_out = midi_out
        .ports()
        .into_iter()
        .find(|x| midi_out.port_name(x).unwrap() == name_out)
        .ok_or(PortOpenError::from_port(name_out))?;
    ...
    loop {
        ...
        port_check_input
            .ports()
            .into_iter()
            .find(|x| port_check_input.port_name(x).unwrap() == name_in)
            .ok_or(PortLostError::from_port(name_in))?;
        let port_check_output = MidiOutput::new("midi-remap out-Port Check")?;
        port_check_output
            .ports()
            .into_iter()
            .find(|x| port_check_output.port_name(x).unwrap() == name_in)
            .ok_or(PortLostError::from_port(name_in))?;
    }
}
...
fn main() {
    ...
    match Mapper::new_from_json_file(matches.value_of("config").unwrap()) {
        ...
        Err(error) => {
            eprintln!("Could not create mapper: {}", error)
        }
    }
}
