...
fn midly_2_param(to: &MsgType, a_8: u8, b_8: u8) -> MidiMessage {
    ...
    match to {
        ...
        MsgType::Control => MidiMessage::Controller {
            controller: a,
            value: b,
        },
    }
        ...
        match in_message {
            MidiMessage::NoteOff { key, vel } => {
                let mappings = self
                    .mappings
                    .iter()
                    .find(|x| x.from == MsgType::NoteOff && x.start <= key && x.stop >= key);
            ...
            MidiMessage::NoteOn { key, vel } => {
                let mappings = self
                    .mappings
                    .iter()
                    .find(|x| x.from == MsgType::NoteOn && x.start <= key && x.stop >= key);
                return match mappings {
                    Some(m) => {
                        midly_2_param(&m.to, key.as_int() + m.to_start - m.start, vel.as_int())
                    }
            ...
            MidiMessage::Aftertouch { key, vel } => {
                let mappings = self
                    .mappings
                    .iter()
                    .find(|x| x.from == MsgType::Aftertouch && x.start <= key && x.stop >= key);
                return match mappings {
                    Some(m) => {
                        midly_2_param(&m.to, key.as_int() + m.to_start - m.start, vel.as_int())
                    }
            ...
            MidiMessage::Controller { controller, value } => {
                ...
                return match mappings {
                    Some(m) => midly_2_param(
                        &m.to,
                        controller.as_int() + m.to_start - m.start,
                        value.as_int(),
                    ),
                    None => in_message,
                };
